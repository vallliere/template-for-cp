...
struct line {
    array<line_t, 2> u, v;
    line() : u({0, 0}), v({0, 0}) {}
    line(line_t x1, line_t y1, line_t x2, line_t y2) : u({x1, y1}), v({x2, y2}) {}
    ...
    static bool is_intersect_with(line_t l1, line_t l2)
    {  // line-point, point-point : O
        ...
        auto is_intersect_ccw = [](array<line_t, 2> a, array<line_t, 2> b, array<line_t, 2> c) {
            ...
            if (cw == 0) {
                x1 = min(a[0], b[0]), x2 = max(a[0], b[0]), y1 = min(a[1], b[1]), y2 = max(a[1], b[1]);
                if (c[0] >= x1 && c[0] <= x2 && c[1] >= y1 && c[1] <= y2) return 0;
                cw = 1;
            }
            return cw;
        };
    ...
    static bool is_intersect_without(line_t l1, line_t l2)
    {  // line-point, point-point : X
        ...
        auto is_on_line = [](line li, array<line_t, 2> c) {
            line_t x1, y1, x2, y2;
            x1 = min(li.v[0], li.u[0]), x2 = max(li.v[0], li.u[0]), y1 = min(li.v[1], li.u[1]), y2 = max(li.v[1], li.u[1]);
        ...
        if (x1 == 0 && y1 == 0 && (is_on_line(l1, l2.u) || is_on_line(l1, l2.u) || is_on_line(l2, l1.u) || is_on_line(l2, l1.v)))
            return 0;
